...
def db_login():
    ...
    login_button = ctk.CTkButton(login_frame, text="Enter", command=lambda: sql_check(login_window, sql_entry, sql_pass))
...
def addinfo_window(frame):
    ...
    female_button: CTkRadioButton = ctk.CTkRadioButton(add_frame, text="Female", variable=gender_var, value="Female")
...
def view_details(frame):
    ...
    def display_table(table, column):
        global data, headers
        headers = ["No", "Names    ", "Class", "Age", "Father's Name", "Gender", "Phone", f"Email ID {" "*20 }", f"Address {" "*20}" ]
...
def edit_student_details(frame):
    ...
    ctk.CTkRadioButton(fields_frame, text="Male", variable=gender_var, value="Male").grid(row=4, column=1, padx=10,
                                                                                          pady=5)
    ctk.CTkRadioButton(fields_frame, text="Female", variable=gender_var, value="Female").grid(row=4, column=2, padx=10,
                                                                                              pady=5)
